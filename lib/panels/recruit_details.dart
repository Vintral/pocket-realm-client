...
class _RecruitDetailsPanelState extends State<RecruitDetailsPanel> {
  ...
  void initState() {    
    ...
    _onRecruitSuccessListener = _provider.on( "RECRUIT_SUCCESS", null, onRecruitSuccess );
  }
  ...
  List<Widget> getCosts() {
    ...
    List<Widget> children = <Widget>[];
    if( _unit.costPoints > 0 ) children.add( buildStat( _unit.costPoints.toString(), "energy" ) );
    if( _unit.costGold > 0 ) children.add( buildStat( _unit.costGold.toString(), "gold" ) );
    if( _unit.costFood > 0 ) children.add( buildStat( _unit.costFood.toString(), "food" ) );
    if( _unit.costWood > 0 ) children.add( buildStat( _unit.costWood.toString(), "wood" ) );
    if( _unit.costStone > 0 ) children.add( buildStat( _unit.costStone.toString(), "stone" ) );
    if( _unit.costMetal > 0 ) children.add( buildStat( _unit.costMetal.toString(), "metal" ) );
    if( _unit.costFaith > 0 ) children.add( buildStat( _unit.costFaith.toString(), "faith" ) );
    if( _unit.costMana > 0 ) children.add( buildStat( _unit.costMana.toString(), "mana" ) );
  ...
  List<Widget> getUpkeeps() {
    ...
    List<Widget> children = <Widget>[];    
    if( _unit.upkeepGold > 0 ) children.add( buildStat( _unit.upkeepGold.toString(), "gold" ) );
    if( _unit.upkeepFood > 0 ) children.add( buildStat( _unit.upkeepFood.toString(), "food" ) );
    if( _unit.upkeepWood > 0 ) children.add( buildStat( _unit.upkeepWood.toString(), "wood" ) );
    if( _unit.upkeepStone > 0 ) children.add( buildStat( _unit.upkeepStone.toString(), "stone" ) );
    if( _unit.upkeepMetal > 0 ) children.add( buildStat( _unit.upkeepMetal.toString(), "metal" ) );
    if( _unit.upkeepFaith > 0 ) children.add( buildStat( _unit.upkeepFaith.toString(), "faith" ) );
    if( _unit.upkeepMana > 0 ) children.add( buildStat( _unit.upkeepMana.toString(), "mana" ) );
                    ...
                    Row(
                      ...
                      children:[
                        Expanded(
                          child: Container(
                            color: _theme.colorBackground,
                            child: Image.asset("assets/units/${_provider.unit?.name ?? "none"}.png", fit: BoxFit.cover, ),
                          ),
                        ),
                        Expanded(
                          child: Padding(
