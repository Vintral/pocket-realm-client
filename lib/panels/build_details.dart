...
class _BuildDetailsPanelState extends State<BuildDetailsPanel> {
  ...
  void initState() {
    ...
    _onBuiltSuccessListener =
        _provider.on("BUILD_SUCCESS", null, onBuildSuccess);
  }
  ...
  Text buildHeader(String headerText) {
    return Text(headerText, style: _theme.textMediumBold);
  }
  ...
  List<Widget> getCosts() {
    ...
    List<Widget> children = <Widget>[];
    if (_building.costPoints > 0) {
      children.add(buildStat(_building.costPoints.toString(), "energy"));
    }
    if (_building.costGold > 0) {
      children.add(buildStat(_building.costGold.toString(), "gold"));
    }
    if (_building.costFood > 0) {
      children.add(buildStat(_building.costFood.toString(), "food"));
    }
    if (_building.costWood > 0) {
      children.add(buildStat(_building.costWood.toString(), "wood"));
    }
    if (_building.costStone > 0) {
      children.add(buildStat(_building.costStone.toString(), "stone"));
    }
    if (_building.costMetal > 0) {
      children.add(buildStat(_building.costMetal.toString(), "metal"));
    }
    if (_building.costFaith > 0) {
      children.add(buildStat(_building.costFaith.toString(), "faith"));
    }
    if (_building.costMana > 0) {
      children.add(buildStat(_building.costMana.toString(), "mana"));
    }
                  ...
                  Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    Expanded(
                      child: Container(
                        color: _theme.colorBackground,
                        child: Image.asset(
                          "assets/buildings/${_provider.building?.name ?? "none"}.png",
                          fit: BoxFit.cover,
                        ),
                      ),
                    ),
