...
class _GatherPanelState extends ListPanelState<GatherPanel> {
  ...
  List<Widget> buildTypes() {
    ...
    var resources = _library.resources.where( ( resource ) => resource.canGather ).toList();
    List<Widget> ret = <Widget>[];
    for( var i = 0; i < resources.length; i++ ) {
      if( i != 0 ) ret.add( SizedBox( width: Settings.horizontalGap, ) );
      _logger.e( "${resources[ i ].name} == ${_provider.resource}" );
      ret.add( ItemWithBorder( item: resources[ i ], handler: onResourceTapped, active: resources[i].name == _library.getResource( _provider.resource ?? "" )?.name, ) );      
    }    
  ...
  Widget buildResults( BuildContext context ) {
    ...
    for( int i = 0; i < _provider.gatherResults.length; i++ ) {
      var image = _library.getResource(_provider.gatherResults[ i ].type)?.name ?? "none";
