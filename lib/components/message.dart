...
class Message extends StatelessWidget {
  ...
  Widget buildImage( double size, bool incoming ) {
    return ItemWithBorder( image: "assets/avatars/${!incoming ? _provider.conversationAvatar : _player.avatar}.png", height: size, reflect: incoming );
  }
          ...
          Expanded(            
            child: Padding(
              ...
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text( data.message, style: _theme.styleShoutMessage ),
                  SizedBox( height: _theme.gap / 2 ),
                  Text( timeSince( data.time ), style: _theme.styleShoutDate ),
                ],
              ),
            ),
          ),          
