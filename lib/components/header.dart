...
class _HeaderState extends State<Header> with TickerProviderStateMixin {
  final Logger _logger = Logger( level: Level.warning, printer: PrettyPrinter( printTime: true ) );
  ...
  late final AnimationController _controllerLowerDrawerOffset = AnimationController(
  ...
  void initState() {
    ...
    _logger.d( "initState" );
    _onPlayerUpdatedListener = _player.on( "UPDATED", null, ( e, o ) => setState( (){} ) );
  }
        ...
        children: [
          Positioned(
            ...
            child: ColorFiltered(
              ...
              child: Image.asset( "assets/ui/header-$image-bar.png", fit: BoxFit.fill, ),
            ),                         
          ),
          Align(
            child: child,
  ...
  Widget buildResource( Resource resource ) {
    ...
    switch( resource.name ) {
      case "gold": quantity = _player.gold.toString(); break;
      case "wood": quantity = _player.wood.toString(); break;
      case "food": quantity = _player.food.toString(); break;
      case "metal": quantity = _player.metal.toString(); break;
      case "stone": quantity = _player.stone.toString(); break;
  ...
  List<Widget> getDrawerContent() {
    ...
    if( _activeTab == Dictionary.get( "RESOURCES" ) ) {
      return [
        ..._library.resources.where((element) => element.name != "mana" && element.name != "faith" ).map( ( resource ) => buildResource( resource ) ),
        ..._library.resources.where((element) => element.name != "mana" && element.name != "faith" ).map( ( resource ) => buildResource( resource ) ),
        ..._library.resources.where((element) => element.name != "mana" && element.name != "faith" ).map( ( resource ) => buildResource( resource ) ),
      ];
    }
    ...
    if( _activeTab == Dictionary.get( "UNITS" ) ) {
      return _player.units
        .where( ( u ) => ( u.quantity >= 1 ) || ( u.quantity > 0 && u.unit!.supportPartial ) )
        .map( ( u ) {
          return ItemWithBorder(
            width: size,
            height: size,
            item: u.unit as Unit,
            quantity: u.quantity < 0 ? "${( u.quantity * 100 ).floor()}%" : u.quantity.floor().toString(),
          );
        } ).toList();
    }
    ...
    if( _activeTab == Dictionary.get( "BUILDINGS" ) ) {
      return _player.buildings
        .where( ( b ) => ( b.quantity >= 1 ) || ( b.quantity > 0 && b.building!.supportPartial ) )
        .map( ( b ) {
          return ItemWithBorder(
            width: size,
            height: size,
            item: b.building as Building,
            quantity: b.quantity < 0 ? "${( b.quantity * 100 ).floor()}%" : b.quantity.floor().toString(),
          );
        } ).toList();
    }
      ...
      child: Stack(
        children: [
          ColorFiltered(
            ...
            child: Image.asset( "assets/ui/drawer-background.png", key: _key, width: _theme.width, height: _theme.width, fit: BoxFit.cover ),
          ),
          Positioned(
            bottom: _theme.gap,
            left: _theme.gap,
  ...
  Widget getDrawer() {
    return Positioned(
      ...
      child: SizedBox(
        height: _theme.headerDrawerCap + ( _offsetLowerDrawerAnimation?.value ?? 0 ),
        child: Stack(
          children:[
            getInnerDrawer(),            
            SizedBox(
  ...
  Widget build(BuildContext context) {
    WidgetsBinding.instance.addPostFrameCallback( (_) {
      ...
      if( _getSize( _key ).height > 0 ) _theme.headerDrawerBackground = _getSize( _key ).height;
    ...
    return Offstage(
      offstage: !_shown,
      child: SizedBox(
        height: _theme.headerHeight + _theme.headerDrawerCap + ( _offsetAnimation?.value ?? 0 ) + ( _offsetLowerDrawerAnimation?.value ?? 0 ),
        child: Stack(
          children:[
            getDrawer(),           
            SizedBox(
                        ...
                        children: [
                          SizedBox(
                            ...
                            child: ColorFiltered(
                              ...
                              child: Image.asset( "assets/ui/header-avatar.png", fit: BoxFit.fitHeight, ),
                            ),
                          ),
                          Padding(
                            padding: EdgeInsets.all( _theme.gap / 2 ),
                            child: SizedBox(                            
                              height: math.max( _theme.headerHeight - _theme.gap, 0 ),
                              child: Image.asset( "assets/avatars/${_player.avatar}.png", fit: BoxFit.fill ),
                            ),
                          ),
                        ],
                      ),
                      Expanded(
                        child: SizedBox(
                          ...
                          child: Column(
                            ...
                            children: [
                              buildCell(
                                child: HeaderStat(icon: "energy", value: _player.energy ),
                                image: "top",
                              ),
                              buildCell( 
                                child: HeaderStat(icon: "land", value: "${_player.land}" ),
                                image: "middle" 
                              ),                              
                              buildCell( 
                                child: HeaderStat(icon: "tree", value: "${_player.landFree}" ),
                                image: "bottom" 
                              ),
                            ],
                          ),
                      ...
                      child: Stack(
                        children: [
                          ColorFiltered(
                            ...
                            child: Image.asset( "assets/ui/panel-header.png", fit: BoxFit.fitWidth, ),
                          ),
                          Center(
