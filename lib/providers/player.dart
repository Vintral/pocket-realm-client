...
class PlayerProvider extends eventify.EventEmitter {
  ...
  final Logger _logger = Logger( level: Level.off, printer: PrettyPrinter( printTime: true ) );
  ...
  void onUpdated( ev, obj ) {
    ...
    if( data[ "user" ] != null ) updateUser( data[ "user" ] );
  }
  ...
  void onEvents( ev, obj ) {
    ...
    Events.addAll( ( ( ev.eventData as dynamic )[ "events" ] as List<dynamic> ).map( ( data ) => EventData( data ) ) );
  ...
  void updateUser( dynamic data ) {
    ...
    for( var key in data.keys ) {
      switch( key ) {
        case "username": username = data[ key ];
        case "avatar": avatar = getIntVal( data[ key ] );
        case "class": characterClass = data[ key ];
        case "character_class": characterClass = data[ key ];        
        case "guid": guid = data[ key ];
        case "round_playing": round = data[ key ];
        case "round_id": roundId = getIntVal( data[ key ] );
        case "energy": energy = getIntVal( data[ key ] );
        case "population": population = getIntVal( data[ key ] );
        case "housing" : population = getIntVal( data[ key ] );        
        case "land": land = getIntVal( data[ key ] );
        case "free_land": landFree = getIntVal( data[ key ] );
        case "gold": gold = getIntVal( data[ key ] );
        case "tick_gold": tickGold = getIntVal( data[ key ] );
        case "food": food = getIntVal( data[ key ] );
        case "tick_food": tickFood = getIntVal( data[ key ] );
        case "wood": wood = getIntVal( data[ key ] );
        case "tick_wood": tickWood = getIntVal( data[ key ] );
        case "metal": metal = getIntVal( data[ key ] );
        case "tick_metal": tickMetal = getIntVal( data[ key ] );
        case "defense": defense = getIntVal( data[ key ] );
        case "stone": stone = getIntVal( data[ key ] );
        case "tick_stone": tickStone = getIntVal( data[ key ] );
        case "faith": faith = getIntVal( data[ key ] );
        case "tick_faith": tickFaith = getIntVal( data[ key ] );
        case "mana": mana = getIntVal( data[ key ] );
        case "tick_mana": tickMana = getIntVal( data[ key ] );
        case "build_power": buildPower = getIntVal( data[ key ] );
        case "recruit_power": recruitPower = getIntVal( data[ key ] );
        case "round": updateUser( data[ key ] );
        case "units": updateUnits( data[ key ] );          
        case "buildings": updateBuildings( data[ key ] );
        case "items": _logger.w( "NYI - Processing Items" );
        default:
    ...
    if( unitData.length > 0 ) { 
      for( dynamic unit in unitData ) {        
        units.add( UserUnit(
          ...
          quantity:unit["quantity"] is int ? ( unit["quantity"] as int ).toDouble() : unit["quantity"],
        ) );            
      }
    }    
  }
    ...
    if( buildingData.length > 0 ) {    
      for( dynamic building in buildingData ) {
        buildings.add( UserBuilding(
          ...
          quantity:building["quantity"] is int ? ( building["quantity"] as int ).toDouble() : building["quantity"],
        ) );      
      }
    }
  }
